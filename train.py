...
# 参数配置
ANNOTATIONS_FILE = "GTZAN_processed\\train_index.csv"  # 测试用训练集索引，可自己修改
ANNOTATIONS_FILE_test = "GTZAN_processed\\test_index.csv"  # 测试集索引，可自己修改
...
model = PRCNN_dpo().to(device) #模型选择，可自行替换
...
def train_model(
    ...
    device: str,
    num_epochs: int = 100,
    log_dir: str = 'logs', # TensorBoard日志目录(记得输)
    save_path: str = None
) -> Dict[str, list]:
    """
    封装完整的模型训练流程
...
if __name__ == "__main__":
    # 训练模型
    train_model(model, train_dataloader, test_dataloader, optimizer=optimizer, device='cuda',num_epochs=100)
